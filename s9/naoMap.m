    ...
    methods
        ...
        function observe_update(map, id, rel_crd, sigma_m)
            ...
            if isempty(idx)
                ...
                map.xi(end+1:end+4) = (map.mu + [rel_crd 0])/sigma_m;
                map.xi(X) = map.xi(X) - (map.mu + [rel_crd 0])/sigma_m;
                
                map.omega(X,X) = map.omega(X,X) + eye(3)/sigma_m; % up left corner
                map.omega(end+1:end+4,end+1:end+4) = eye(3)/sigma_m; % low right corner
            ...
            map.xi(L) = map.xi(L) + (map.mu + [rel_crd 0])/sigma_m;
            map.xi(X) = map.xi(X) - (map.mu + [rel_crd 0])/sigma_m;
